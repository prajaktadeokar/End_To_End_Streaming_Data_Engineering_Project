...
@dp.append_flow(
  target = "uber_catalog.silver_staging.staging_rides" )
def rides_stream():
    df= spark.readStream.table("uber_catalog.bronze.rides_raw_data")
    df_parsed = df.withColumn("parsed_rides", from_json(col("rides"), df_rides_raw_schema)).select("parsed_rides.*")
    return df_parsed
